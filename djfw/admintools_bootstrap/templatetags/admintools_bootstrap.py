...
@register.simple_tag
def atb_site_link():
    site_link = getattr(settings, 'ADMINTOOLS_BOOTSTRAP_SITE_LINK', None)
    if (not site_link) and ('django.contrib.sites' in settings.INSTALLED_APPS):
        from django.contrib.sites.models import Site
        site_link = "http://%s/" % (Site.objects.get_current().domain, )
    if (not site_link):
        site_link = '/'
    return '''
        <li><a href="%s"  class="top-icon" title="%s" rel="popover" data-placement="below"><i
            ...
            ''' % (site_link, _('Open site'))

@register.simple_tag
def atb_tinymce_enabled():
    if 'djfw.tinymce' in settings.INSTALLED_APPS:
        c = Context({})
        t = loader.get_template('tinymce/init_advanced_editor.html')
        return t.render(c)
    return ""
