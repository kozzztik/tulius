    ...
    def languaged_ids(self):
        ...
        for block in full_list:
            ...
            for subblock in filtered_list:
                if (subblock[0] == block.name) and (subblock[1] == block.urls) and (subblock[2] == block.exclude_urls):
                    if (block.language == lang) or (block.language == None ):
                        subblock[3] = block.language
                        subblock[4] = block.id
                    found = True
                    break
            if not found:
                filtered_list += [[block.name, block.urls, block.exclude_urls, block.language, block.id]]
