...
def_chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890'
...
def do_gauss_analyze(query, field_name, intervals):
    values = query.aggregate(
        model_min=models.Min(field_name),
    ...
    for i in range(intervals):
        ...
        data += [
            (
                '%s - %s' % (interval_start, interval_end),
                query.filter(**subquery).count()
            )]
    return data
