...
class Smiles(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not emotions:
            ...
            with transaction.commit_on_success():
                for emotion in emotions_list:
                    emotion_obj = Emotion(name=emotion['name'], text=emotion['text'], filename=emotion['filename']) 
                    emotion_obj.save()
            emotions = Emotion.objects.all()
        emotions_list = {}
        new_emotions = []
        ...
        emotions = []
        for i in xrange(0, len(new_emotions)/10 + 1):
...
class Uploaded_files(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        if not self.request.user.is_anonymous():
            all_files = FileUpload.objects.filter(user=self.request.user).order_by('-id')[:15]
            all_files = [afile for afile in all_files if afile.body]
        else:
            all_files = []
        files = []
        for i in xrange(0, len(all_files)/5 + 1):
            files += [all_files[5*i:5*i+5]]
        return {'files': files}

def save_upload(request, upload, filename):
    uploaded_file = FileUpload(user=request.user, filename=filename, 
        file_length=len(upload), mime=guess_type(filename, True)[0])
