...
from .forms import ChangeMaintainceForm, OperationsForm
...
def on_maintaince_view(
        request, obj, template_name='installer/on_maintaince.html'):
...
class AddMaintainceView(FormView):
    ...
    success_url = urls.reverse_lazy(
        'admin:installer_maintenancelog_changelist')
    success_url2 = urls.reverse_lazy(
        'admin:installer_maintenancelog_changelist')
    form_url = ''
    
    def render_to_response(self, context, **response_kwargs):
        return self.modeladmin.render_change_form(
            self.request, context, form_url=self.form_url, add=True)
    ...
    def get_context_data(self, **kwargs):
        ...
        form = kwargs['form']
        context = {
            'title': _('Start %s') % opts.verbose_name,
    ...
    def form_valid(self, form):
        opts = [
            key for key in form.cleaned_data.keys() if (key != 'comment')
            and form.cleaned_data[key]]
        log = MaintenanceLog()
        log.status = _("Maintaince started") if opts else \
            _("Manual maintaince")
        ...
        if opts:
            ...
            return HttpResponseRedirect(
                urls.reverse(
                    'admin:installer_maintenancelog_change', args=(log.pk,)))
...
class ChangeMaintainceView(AddMaintainceView):
    ...
    def get_form(self, form_class=None):
        ...
        if (not form) or getattr(form, 'reload', False):
            self.add_js_media += [
                u'%sinstaller/reload.js' % settings.STATIC_URL]
        else:
            pass
        if not form:
            form = form_class(
                self.request.POST or None,
                self.request.FILES or None, instance=self.obj)
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            urls.reverse(
                'admin:installer_maintenancelog_change', args=(self.obj.pk,)))
        # return HttpResponseRedirect(self.get_success_url())
