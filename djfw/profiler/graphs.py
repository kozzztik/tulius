...
def time_period_graph(request, *args, **kwargs):
    ...
    def annotate_stats(query):
        if weight_field:
            return query.annotate(weight_count=Count(weight_field), weight_sum=Sum(weight_field), weight_avg=Avg(weight_field))
        else:
            return query.annotate(weight_sum=Count('id'))
        
    delta = (endtime - starttime) / intervals
    data = []
    period_query = ProfilerMessage.objects.filter(create_time__lte=endtime,create_time__gte=starttime)
    modules = period_query.order_by('module_name').distinct().values('module_name')
    ...
    for module in modules:
        functions = period_query.filter(module_name=module['module_name']).order_by('func_name').distinct()
    ...
    for i in range(intervals):
        ...
        values = period_query.order_by('module_name').distinct().values('module_name')
        if weight_field:
    ...
    graph = {'data': data, 'modules': module_list, 'details': weight_field,
             'modules_sum': all_modules_sum, 'modules_count': all_modules_count, 'modules_avg': all_modules_avg}
    return graph
...
def sunlignt_graph(request, category_field, category_subfield=None):
    ...
    period_query = ProfilerMessage.objects.filter(create_time__lte=endtime,create_time__gte=starttime)
    all_weight = 0
    categories = period_query.order_by(category_field).distinct().values(category_field)
    if weight_field:
    ...
    for category_obj in categories:
        ...
        category['sublabel'] =  ("%s %s (%.1f%%)" % (unicode(_("Views:")), category_weight, category['percentage']))
        if category_subfield:
            category_elems = []
            subquery = {}
            subquery[category_field] = category_id
            elems = period_query.filter(**subquery).order_by(category_subfield).distinct().values(category_subfield)
            if weight_field:
            ...
            for elem in elems:
                ...
                category_elem['percentage'] = category_elem_weight * 100.0 / category_weight
                category_elem['percentage_total'] = category_elem_weight * 100.0 / all_weight
                category_elem['id'] = "%s %s" % (category_id, elem[category_subfield])
                category_elem['name'] = elem[category_subfield] or "-"
                category_elem['label'] = category_id + " " + (elem[category_subfield] or "") 
                category_elem['sublabel'] = unicode(_("Views:")) +\
                    "%s (%s: %.1f%%, "  % (category_elem_weight, category_id, category_elem['percentage'],) +\
                    unicode(_("All:")) +" %.1f%%)" % (category_elem['percentage_total'],)
                category_elems += [category_elem]
            category['data'] = category_elems
        data += [category]
    return {'weight': all_weight, 'data': data, 'category_name': category_field, 'id': category_field, 
            'weight_name': request.profiler_period.weight_title, 'category_subfield': category_subfield}
    
def distribution_graph(request, module_name=None, function_name=None, intervals=20):
    endtime = request.profiler_period.end
    starttime = request.profiler_period.start
    weight_field = request.profiler_period.weight_field
    if not weight_field:
    ...
    data = []
    period_query = ProfilerMessage.objects.filter(create_time__lte=endtime,create_time__gte=starttime)
    values = period_query.aggregate(weight_min=Min(weight_field), weight_max=Max(weight_field))
...
def collapsed_time_period_graph(start_day, days, weight_field, multi=1):
    ...
    for i in range(intervals):
        ...
        if days <= PARTING_DAYS:
            query = query.filter(create_time__gte=interval_start, create_time__lt=interval_end)
...
def collapsed_sunlignt_graph(start_day, days, weight_field, subfield=None, mappings={}, strict_mappings=False):
    category_subfield =  weight_field <> 'devices'
    categories = {}
    for i in range(days):
        try:
            day_data = ClientCollapse.objects.get(day=start_day + datetime.timedelta(days=i))
        except:
            day_data = None
        if day_data:
            ...
            for subcategory in categories[category].keys():
                ...
                if mappings and category in mappings:
                    if subcategory in mappings[category]['items']:
                        elem['label'] = mappings[category]['items'][subcategory]
                    elif strict_mappings:
                        continue
                subdata += [elem]
                weight += subweight
            c['data'] = sorted(subdata, key=lambda x: x['weight'], reverse=True)
        else:
    ...
    for category in data:
        ...
        if category_subfield:
            for subcategory in category['data']:
                ...
                subcategory['percentage'] = sub_weight * 100.0 / category_weight
                subcategory['percentage_total'] = sub_weight * 100.0 / all_weight
                subcategory['sublabel'] = unicode(_("Views:")) +\
                    "%s (%s: %.1f%%, "  % (sub_weight, category['label'], subcategory['percentage'],) +\
                    unicode(_("All:")) +" %.1f%%)" % (subcategory['percentage_total'],)
    data = sorted(data, key=lambda x: x['weight'], reverse=True)
    return {'weight': all_weight, 'data': data, 'category_name': weight_field, 'id': weight_field, 
            'category_subfield': category_subfield}
    
def collapsed_sum_graph(start_day, days, weight_fields, captions, incremental=False, multi=1):
    intervals = days if days > PARTING_DAYS else COLLAPSE_INTERVALS * days
    delta = datetime.timedelta(days=1)
    if days <= PARTING_DAYS:
        delta = delta / COLLAPSE_INTERVALS
