...
class LastTime(object):
    def __init__(self, offset_days=0, offset_month=0, round_day=False, round_month=False, 
                 round_week=False):
...
WEIGHT_SELECTION = (
    ...
    (WEIGHT_SELECTOR_DB_COUNT,  _(u'DB queries count'), 'db_count', _('Count')),
    (WEIGHT_SELECTOR_TEMPLATE_TIME,  _(u'Template rendering time'), 'template_time', _('Time')),
    (WEIGHT_SELECTOR_TEMPLATE_DB_TIME,  _(u'Template DB work time'), 'template_db_time', _('Time')),
    (WEIGHT_SELECTOR_TEMPLATE_DB_COUNT,  _(u'Template DB queries count'), 'template_db_count', _('Count')),
...
TIME_SELECTION = (
    ...
    (TIME_SELECTOR_LAST_MONTH, _(u'last month'), LastTime(0, 1), LastTime(), 64),
    (TIME_SELECTOR_TODAY, _(u'today'), LastTime(1, round_day=True), LastTime(round_day=True), 96),
    (TIME_SELECTOR_THISWEEK, _(u'this week'), LastTime(7, round_week=True), LastTime(round_week=True), 84),
    (TIME_SELECTOR_THISMONTH, _(u'this month'), LastTime(0, 1, round_month=True), LastTime(round_month=True), 64),
    (TIME_SELECTOR_PRDAY, _(u'previous day'), LastTime(2, round_day=True), LastTime(1, round_day=True), 96),
    (TIME_SELECTOR_PRWEEK, _(u'previous week'), LastTime(14, round_week=True), LastTime(7, round_week=True), 84),
    (TIME_SELECTOR_PRMONTH, _(u'previous month'), LastTime(0, 2, round_month=True), LastTime(0, 1, round_month=True), 64),
)
...
class TimePeriodView(TemplateView):
    def update_request(self, request):
        ...
        if PROFILER_WEIGHT_SELECTOR in request.session:
            weight = WEIGHT_SELECTION[request.session[PROFILER_WEIGHT_SELECTOR]]
...
class SetTimePeriod(TimePeriodView):
    ...
    def get_context_data(self, **kwargs):
        ...
        weight_choices = [(weight[0], weight[1]) for weight in WEIGHT_SELECTION]
        self.form = TimeSelectorForm(period_choices, weight_choices, data=self.request.POST or None, 
                            initial={'period': self.request.profiler_period.num, 'weight': self.request.profiler_period.weight_num})
    ...
    def post(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(reverse('admin:profiler_profilermessage_changelist'))
