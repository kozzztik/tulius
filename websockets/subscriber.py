...
class RedisSubscriber(RedisStore):
    ...
    def set_pubsub_channels(self, request, channels):
        ...
        for key in self._get_message_channels(
                request=request, facility=facility, **audience):
    ...
    def get_file_descriptor(self):
        ...
        """
        return self._subscription.connection and \
            self._subscription.connection._sock.fileno()

    def release(self):
        """
        New implementation to free up Redis subscriptions when websockets
        close. This prevents memory sap when Redis Output Buffer and Output
        Lists build when websockets are abandoned.
        """
