...
class RedisPublisher(RedisStore):
    def __init__(self, **kwargs):
        """
        Initialize the channels for publishing messages through
        the message queue.
        """
    ...
    def fetch_message(self, request, facility, audience='any'):
        ...
        The current HTTP ``request`` is used to determine to whom the
         message belongs.
        A unique string is used to identify the bucket's ``facility``.
        Determines the ``audience`` to check for the message. Must be one
        of ``broadcast``,
        ``group``, ``user``, ``session`` or ``any``. The default is ``any``,
        which means to check for all possible audiences.
        """
        ...
        if audience in ('session', 'any',):
            if request and request.session:
                channels.append('{prefix}session:{0}:{facility}'.format(
                    request.session.session_key, prefix=prefix,
                    facility=facility))
        if audience in ('user', 'any',):
            if request and request.user and request.user.is_authenticated:
                channels.append('{prefix}user:{0}:{facility}'.format(
                    request.user.pk, prefix=prefix, facility=facility))
