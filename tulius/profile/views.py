...
from .forms import NotificationForm, ChangeEmailForm, ProfileSettingsForm, PersonalSettingsForm
...
class PlayerSettingsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['personal_settings_form'] = ProfileSettingsForm(instance=self.request.user)
        kwargs['settings_form'] = PersonalSettingsForm(instance=self.request.user)
        return kwargs
...
class PlayerSubscriptionsView(LoginTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for n in notifications:
            form = NotificationForm(prefix='n'+str(n.id), initial={'enabled': n not in user_n})
            form.notification = n
            forms += [form]
        return locals()
        ...
        for form in forms:
            if form.is_valid():
                ...
                if enabled:
                    ...
                    n = UserNotification.objects.get_or_create(user=self.request.user,
                                                               notification=form.notification)[0]
                    n.enabled = False
                    n.save()
        return HttpResponseRedirect(reverse('profile:subscriptions'))
...
class InvitesView(LoginTemplateView):
    ...
    def get_context_data(self, **kwargs):
        kwargs['new_invites'] = GameInvite.objects.filter(user=self.request.user, status=GAME_INVITE_STATUS_NEW)
        kwargs['old_invites'] = GameInvite.objects.filter(user=self.request.user).exclude(status=GAME_INVITE_STATUS_NEW)
        return kwargs
...
class PlayerInviteBaseView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(reverse('profile:invites'))
