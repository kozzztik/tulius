...
def save_illustration(request, upload, filename, story, variation, illustration):
    """
    ...
    if not illustration:
        illustration = Illustration(story=story, variation=variation, name=os.path.splitext(filename)[0])
        illustration.save()
...
def upload_illustration(request, story, variation, illustration):
    return handle_upload(request, save_illustration, story, variation, illustration)
...
def get_illustration_file(request, illustration_id, is_thumb):
    ...
    if (illustration.admins_only) and (not illustration.edit_right(request.user)):
        raise Http404()
    if is_thumb:
