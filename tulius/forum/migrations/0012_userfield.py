...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for model_name in ['UploadedFile', 'Thread', 'ThreadAccessRight', 'ThreadCollapseStatus', 'Comment', 'ThreadReadMark',
                           'CommentLike', 'ThreadDeleteMark', 'CommentDeleteMark', 'OnlineUser', 'Voting', 'VotingVote']:
            orm['forum.' + model_name].objects.all().update(new_user=models.F('user'))
            if model_name == 'Comment':
                orm['forum.' + model_name].objects.all().update(new_editor=models.F('editor'))

    def backwards(self, orm):
        for model_name in ['UploadedFile', 'Thread', 'ThreadAccessRight', 'ThreadCollapseStatus', 'Comment', 'ThreadReadMark',
                           'CommentLike', 'ThreadDeleteMark', 'CommentDeleteMark', 'OnlineUser', 'Voting', 'VotingVote']:
            orm['forum.' + model_name].objects.all().update(user=models.F('new_user'))
            if model_name == 'Comment':
                orm['forum.' + model_name].objects.all().update(editor=models.F('new_editor'))
