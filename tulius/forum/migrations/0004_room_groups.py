...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if not users:
            return
        superuser = users[0]
        threads = Thread.objects.filter(parent=None, plugin_id=None)
        for thread in threads:
            thread.migrated = False
        for room_group in orm.RoomGroup.objects.all():
            thread_room = Thread(parent=None, title=room_group.title, access_type=1, user=superuser, room=True, plugin_id=None)
            thread_room.save()
            for group_link in orm.ThreadGroupLink.objects.filter(group=room_group):
                for thread in threads:
                    if thread.id == group_link.thread_id:
                        thread.parent = thread_room
                        thread.save()
                        thread.migrated = True
                group_link.delete()
            room_group.delete()
        not_sorted = [thread for thread in threads if not thread.migrated]
        if not_sorted:
            thread_room = Thread(parent=None, title='Not sorted', access_type=1, user=superuser, room=True, plugin_id=None)
            thread_room.save()
            for thread in not_sorted:
                thread.parent = thread_room
                thread.save()
