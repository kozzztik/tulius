...
class BaseCommentView(BasePluginView):
    def get_context_data(self, **kwargs):
        ...
        self.comment_id = kwargs['comment_id'] if 'comment_id' in kwargs else None
        self.comment = None
        self.reply = None
        if not (reply_id or self.comment_id):
            raise Http404()
        if reply_id:
            (self.parent_thread, self.reply) = self.core.get_parent_comment(self.request.user, reply_id, True)
        else:
            (self.parent_thread, self.comment) = self.core.get_parent_comment(self.request.user, self.comment_id, False)
...
class EditComment(BaseCommentView):
    ...
    def get_context_data(self, **kwargs):
        ...
        self.site.signals.comment_before_edit.send(self, comment=self.comment, context=context)
        (form, comment) = self.core.process_edit_comment(self.request, self.parent_thread, self.comment, self.reply, True, self.edit_is_valid)
        context['form'] = form
        context['comment'] = comment
        self.comment = comment
        if not self.comment_id:
            context['form_submit_title'] = _("add")
        self.site.signals.comment_after_edit.send(self, comment=self.comment, context=context, adding=(self.comment_id is None))
        return context
...
class FastReply(BaseCommentView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ret_json = {'error_message': unicode(error_message), 'form': html}
...
class CommentsPage(BasePluginView):
    ...
    def get_context_data(self, **kwargs):
        ...
        self.parent_thread = self.core.get_parent_thread(self.request.user, kwargs['thread_id'], False)
        context['parent_thread'] = self.parent_thread
        self.page_num = self.request.GET['page_num'] if 'page_num' in self.request.GET else 1
        ...
        self.comments = self.core.get_comments_page(self.request.user, self.parent_thread, self.page_num)
        context['comments'] = self.comments
        return context

    def get(self, request, *args, **kwargs):
        html = self.render(**kwargs)
        pagination_context = get_pagination_context(request, self.page_num, self.pages)
        ...
        ret_json = {'html': html, 'pagination': pagination_top}
        ret_json['pagination_bottom'] = pagination_bottom
        self.site.signals.view_comments_page.send(self, json=ret_json, comments=self.comments, page=self.page_num, 
...
class CommentRedirrect(BaseCommentView):
    def get(self, request, *args, **kwargs):
        ...
        if self.comment.deleted:
            raise Http404(unicode(_('Comment was deleted')))
        return HttpResponseRedirect(self.site.urlizer.comment_paged(self.comment))
...
class Preview(BasePluginView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['body'] = self.request.POST['body'] if 'body' in self.request.POST else ''
        context['title'] = self.request.POST['title'] if 'title' in self.request.POST else ''
...
class DeleteComment(BasePluginView):
    def post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            ...
            (success, error_text, redirect, text) = self.core.delete_comment(request.user, comment_id, message)
        return HttpResponse(json.dumps({'result': success, 'error_text': unicode(error_text), 'redirect': redirect,
                                        'text': unicode(text)
                                        }))