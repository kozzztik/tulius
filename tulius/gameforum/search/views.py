...
class ExtendedSearchView(BasePluginView, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (not variation.game) and (
                not variation.edit_right(self.request.user)):
            raise PermissionDenied()
        if variation.game and (
                not variation.game.read_right(self.request.user)):
            raise PermissionDenied()
        roles = Role.objects.filter(
            variation=variation, show_in_character_list=True)
        threads = self.core.threads_search_list(
            self.request.user, variation.thread)
        threads = [variation.thread] + threads
        threads = [
            (thread.id, '- ' * thread.level + thread.title)
            for thread in threads]
        self.form = ExtendedSearchForm(
            roles, threads, data=self.request.POST or None)
...
class SearchView(ExtendedSearchView):
    ...
    def get_context_data(self, **kwargs):
        ...
        conditions = []
        parent_thread = self.site.core.get_parent_thread(
            self.request.user, variation.thread_id)
        ...
        if filter_roles:
            conditions += [_(u'From characters: ') + ', '.join(
        ...
        if filter_not_roles:
            conditions += [
                _(u'Not from characters: ') + ', '.join(
                    [role.name for role in filter_not_roles])]
            comments = comments.exclude(
                data1__in=[role.id for role in filter_not_roles])
