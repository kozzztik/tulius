...
from .models import Game, RoleRequest, RoleRequestSelection, RequestQuestion, RequestQuestionAnswer
...
from .game_edit_catalog import EDIT_GAME_PAGES_REQUESTS, EDIT_GAME_PAGES_ROLES, EditGameSubpage
...
class RequestSelectionForm(forms.models.ModelForm):
    ...
    def after_constuct(self, formset, params, i):
        ...
        self.fields['role'].queryset = \
            self.fields['role'].queryset.filter(variation=game.variation, requestable=True).exclude(deleted=True)
            
@login_required
def make_game_request(request, game_id, template_name='games/game_request.haml'):
    """
    ...
    #answers_Formset = get_request_answer_formset(extra=questions.count(), can_delete=False)
    answersformset = get_formset(RoleRequest, RequestQuestionAnswer, request.POST, RequestAnswerForm, static=True, 
        extra=len(questions), params={'game': game, 'questions': questions})
    selectionformset = get_formset(RoleRequest, RoleRequestSelection, request.POST, RequestSelectionForm, 
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and all_valid:
            ...
            for role in rolelist:
                selection = RoleRequestSelection(role_request=role_request, prefer_order=0, role=role)
                selection.save()
            messages.success(request, _('Game request was successfully created'))
...
@login_required
def cancel_game_request(request, game_id, template_name='games/game_request.haml'):
    """
    Making game request view
    """
    ...
    for role_request in role_requests:
        ...
        RequestQuestionAnswer.objects.filter(role_request=role_request).delete()
        roles = Role.objects.filter(variation=game.variation, user=request.user)
        for role in roles:
            role.user = None
            role.save()
        role_request.delete()
...
def role_assign_user(request, role_id, user_id, backtoroles=False):
    ...
    role.user = user
    role.save()
    GameInvite.objects.filter(role=role).update(status=GAME_INVITE_STATUS_OCCUPIED)
    if backtoroles:
        back = EDIT_GAME_PAGES_ROLES
    else:
        back = EDIT_GAME_PAGES_REQUESTS
    return HttpResponseRedirect(EditGameSubpage(role.variation.game, url=back).url)
...
def role_clear_user(request, role_id):
    ...
    role.user = None
    role.save()
    return HttpResponseRedirect(EditGameSubpage(role.variation.game, url=EDIT_GAME_PAGES_ROLES).url)
